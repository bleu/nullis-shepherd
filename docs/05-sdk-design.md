...
│   ├── transport.rs          # HostTransport — alloy Transport impl over rpc::request
...
pub use crate::bindings::shepherd::core::rpc;
...
Modules frequently need to read chain state and encode/decode Ethereum calldata. The SDK provides a full alloy `Provider` (via `HostTransport` over `rpc::request`) and integrates `alloy-sol-types` and `alloy-primitives` (compiled to WASM):
...
These are already WASM-compatible (no-std support, no system dependencies). The `HostTransport` routes all RPC calls through the single `rpc::request` host function — see doc 07 for the full design.
